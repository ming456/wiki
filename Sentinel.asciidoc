...
> 注：一般推荐将 `@SentinelResource` 注解加到服务上，而在 Web 层直接使用 Spring Cloud Alibaba 自带的 Web 适配埋点。Sentinel Web 适配支持配置自定义流控处理逻辑，参考 https://github.com/alibaba/Sentinel/wiki/主流框架的适配#web-适配[相关文档]。
